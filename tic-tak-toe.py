...
def tic_tok():
	...
	def check_win():
		if (block_dict[1] == block_dict[2]):
			if block_dict[1] == block_dict[3]:
				return True		

		if (block_dict[1] == block_dict[4]):
			if block_dict[1] == block_dict[7]:
				return True		

		if (block_dict[1] == block_dict[5]):
			if block_dict[1] == block_dict[9]:
				return True	 		

		if (block_dict[7] == block_dict[8]):
			if block_dict[7] == block_dict[9]:
				return True	

		if (block_dict[7] == block_dict[5]):
			if block_dict[7] == block_dict[3]:
				return True	

		if (block_dict[3] == block_dict[6]):
			if block_dict[3] == block_dict[9]:
				return True	
			
		if (block_dict[4] == block_dict[5]):
			if block_dict[4] == block_dict[6]:
				return True	
	
		if (block_dict[2] == block_dict[5]):
			if block_dict[2] == block_dict[8]:
				return True	

		return None


	def callback(var, play):
		#print("play is: " + play)
		global player, player_name, b1
